// 风险点详情
<template>
  <div class="disasterdetail">
    <div class="mid-nav">风险点详情</div>
    <div class="supTop-nav">
      <i class="el-icon-arrow-left" @click="goBack" />
      <div @click="toFeedBack">信息反馈</div>
    </div>
    <div class="content">
      <div class="content-top">
        <p class="content-title">基本信息</p>
        <div class="info-title">灾害点名称</div>
        <div class="info-content">{{ data.name }}</div>
        <div class="mid-box">
          <div>
            <div class="info-title">灾害类型</div>
            <div class="info-content">{{ data.disasterTypeStr }}</div>
          </div>
          <div>
            <div class="info-title">是否重点监测点位</div>
            <div class="info-content">
              {{ data.isMonitoring ? "是" : "否" }}
            </div>
          </div>
        </div>
        <div class="info-title">始发时间</div>
        <div class="info-content">
          {{ data.happenTime ? data.happenTime : "-" }}
        </div>
      </div>
      <div class="content-top">
        <p class="content-title">地理信息</p>
        <div class="info-title">行政区划</div>
        <div class="info-content">
          {{ data.districtName ? data.districtName : "-" }}
        </div>
        <div class="info-title">地理位置</div>
        <div class="info-content">{{ data.address ? data.address : "-" }}</div>
        <div class="mid-box">
          <div>
            <div class="info-title">经度</div>
            <div class="info-content">
              {{ data.longitude ? data.longitude : "-" }}
            </div>
          </div>
          <div>
            <div class="info-title">纬度</div>
            <div class="info-content">
              {{ data.latitude ? data.latitude : "-" }}
            </div>
          </div>
        </div>
        <div class="info-title">灾害点地图</div>
        <!-- <el-image class="info-location" /> -->
        <div id="egismap" class="mapBG"></div>
      </div>
      <div class="content-top">
        <p class="content-title">管控信息</p>
        <div class="info-title">责任单位</div>
        <div class="info-content">
          {{ data.responsibleUnit ? data.responsibleUnit : "-" }}
        </div>
        <div class="mid-box">
          <div>
            <div class="info-title">分管领导</div>
            <div class="info-content">
              {{ data.leaders ? data.leaders : "-" }}
            </div>
          </div>
          <div>
            <div class="info-title">分管领导电话</div>
            <div class="info-content">
              {{ data.leadersPhone ? data.leadersPhone : "-" }}
            </div>
          </div>
        </div>
        <div class="mid-box">
          <div>
            <div class="info-title">防灾责任人姓名</div>
            <div class="info-content">
              {{ data.disasterDutyName ? data.disasterDutyName : "-" }}
            </div>
          </div>
          <div>
            <div class="info-title">防灾责任人电话</div>
            <div class="info-content">
              {{ data.disasterDutyPhone ? data.disasterDutyPhone : "-" }}
            </div>
          </div>
        </div>
        <div class="info-title">防灾措施建议</div>
        <div class="info-content">{{ data.suggest ? data.suggest : "-" }}</div>
        <div class="mid-box">
          <div>
            <div class="info-title">监测责任人姓名</div>
            <div class="info-content">
              {{ data.monitorDutyName ? data.monitorDutyName : "-" }}
            </div>
          </div>
          <div>
            <div class="info-title">监测责任人电话</div>
            <div class="info-content">
              {{ data.monitorDutyPhone ? data.monitorDutyPhone : "-" }}
            </div>
          </div>
        </div>
        <div class="mid-box">
          <div>
            <div class="info-title">监测人</div>
            <div class="info-content">
              {{ data.monitorName ? data.monitorName : "-" }}
            </div>
          </div>
          <div>
            <div class="info-title">监测人电话</div>
            <div class="info-content">
              {{ data.monitorPhone ? data.monitorPhone : "-" }}
            </div>
          </div>
        </div>
        <div class="info-title">现场图片</div>
        <div
          v-if="data.disasterRiskFiles && data.disasterRiskFiles.length > 0"
          class="imgs-box"
        >
          <van-image
            :src="format.riskphoto(i.id)"
            alt="imgs"
            class="imgs"
            v-for="(i, index) in data.disasterRiskFiles"
            @click="openPhoto(data.disasterRiskFiles, index)"
            :key="i.id"
            fit="cover"
          >
            <template v-slot:loading>
              <div class="loading-img">图片加载中</div>
            </template>
          </van-image>
        </div>
        <span class="no-photo" v-else>无图片</span>
      </div>
    </div>
  </div>
</template>

<script>
import { ImagePreview } from "vant";

export default {
  name: "DisasterDetail",
  data() {
    return {
      data: "",
      longitude: "", // 经度
      latitude: "" // 纬度
    };
  },
  mounted() {
    this.getDetail();
  },
  methods: {
    goBack() {
      this.$router.go(-1);
    },
    showMap() {
      let egis = window.egis;
      let that = this;
      var urlWMTS = "http://120.52.31.31:590/service/api/egis/base/v1/wmts"; //wmts服务地址
      var client_id = "4441c1fa1b004b58a4eabdb255d966cc"; //用户id
      var client_secret = "46d3c60424e841d8b0ebb9e0a14cb43e"; //用户密码
      // var tokenUrl = "http://120.52.31.31:590/oauth/token"; //授权服务地址
      // var authType = "Token"; //授权类型
      // 创建地图
      var center = [that.longitude, that.latitude];
      var egismap = new egis.carto.Map({
        defaultExtent: {
          center,
          maxZoom: 15,
          minZoom: 15,
          level: 15,
          extent: [-180, -90, 180, 90],
          projection: "EPSG:4490"
        }
      });
      // 初始化地图，传入要初始化的DOM对象的id
      egismap.init({ targetId: "egismap" });
      var resthttp = new egis.core.RestHttp({
        client_id: client_id,
        client_secret: client_secret
      });
      // 创建 天地图矢量 瓦片图层
      var tiandituvec = new egis.carto.TileLayer({
        restHttp: resthttp,
        name: "天地图矢量",
        layers: "vec", //": "图层名称",
        matrix: 21, //": "切图级别小于等于切图级别",
        matrixSet: "c", //": "切图策略",
        matrixPrefix: "", //": "切图策略加冒号：",
        format: "tiles", //": "图层格式",
        projection: "EPSG:4490", //": "投影参考",
        layerType: 1, //": "图层类型",
        tileType: 102, //": "瓦片类型",
        opacity: 1.0, //": "透明度",
        visible: true, //": "是否显示",
        crossOrigin: "anonymous",
        style: "default",
        extent: { minx: -180, miny: -90, maxx: 180, maxy: 90 },
        wrapX: true, //": "是否展示循环图",
        url: urlWMTS
      });

      // 创建 天地图中文标注 瓦片图层
      var tianditucta = new egis.carto.TileLayer({
        restHttp: resthttp,
        name: "天地图中文标注",
        layers: "cva", // 图层名称
        matrixSet: "c", // 切图策略
        format: "tiles", // 图层格式
        projection: "EPSG:4490",
        extent: { minx: -180, miny: -90, maxx: 180, maxy: 90 },
        matrixPrefix: "",
        matrix: 21,
        tileType: 102, // 瓦片类型
        opacity: 1.0,
        visible: true,
        wrapX: true, //": "是否展示循环图",
        url: urlWMTS // 图层服务 url
      });
      //添加天地图矢量图层
      egismap.addLayer(tiandituvec);
      //添加天地图中文标注图层
      egismap.addLayer(tianditucta);
      var elementLayer = new egis.carto.ElementLayer({
        name: "元素图层",
        id: "elementLayer",
        visible: true,
        opacity: 1
      });
      egismap.addLayer(elementLayer);

      var picturemarkersymbol = new egis.sfs.PictureMarkerSymbol({
        source:
          "data:image/png;base64,",
        width: 50,
        height: 60,
        rotation: 0,
        opacity: 1,
        offsetX: 0,
        offsetY: 15,
        scale: 0.5
      });
      //创建图片标记面元素
      var linearElement = new egis.sfs.Element({
        geometry: new egis.sfs.Point({
          x: center[0],
          y: center[1],
          spatialReference: 4490
        }),
        symbol: picturemarkersymbol
      });
      elementLayer.add(linearElement);
      egismap.removeInteractions();
    },
    // 获取详情
    getDetail() {
      let id = sessionStorage.getItem("riskinfoId");
      this.axios({
        url: "/natural/disaster/disk/data",
        method: "GET",
        params: {
          id
        }
      })
        .then(res => {
          this.data = res.data.data;
          this.longitude = res.data.data.longitude;
          this.latitude = res.data.data.latitude;
          this.showMap();
        })
        .catch(err => {
          console.log(err);
        });
    },
    // 信息反馈
    toFeedBack() {
      let obj = {
        visible: false,
        content: "",
        img: [],
        video: []
      };
      sessionStorage.setItem("feedBack", JSON.stringify(obj));
      this.$router.push({
        path: "/feedback"
      });
    },
    // 图片预览
    openPhoto(item, index) {
      let arr = [];
      for (let i = 0; i < item.length; i++) {
        arr.push(this.format.photo(item[i].id));
      }
      ImagePreview({
        images: arr,
        startPosition: index,
        closeable: true
      });
    }
  }
};
</script>

<style lang="less" scoped>
.disasterdetail {
  width: 100vw;
  // background: white;
  display: flex;
  flex-direction: column;
  min-height: 100vh;
  position: relative;
  .el-icon-arrow-left {
    font-size: 20px;
  }
  .supTop-nav {
    width: 100%;
    height: 50px;
    display: flex;
    align-items: center;
    padding: 0 15px;
    justify-content: space-between;
    color: white;
  }
  .mid-nav {
    width: 100%;
    height: 50px;
    background-color: #0199ff;
    position: absolute;
    color: white;
    top: 0;
    left: 0;
    text-align: center;
    line-height: 50px;
    z-index: -1;
  }
  .content {
    width: 100%;
    display: flex;
    flex-direction: column;
    // flex: 1;
    // overflow-x: hidden;
    background: white;
    .info-title {
      color: #9f9f9f;
      font-size: 13px;
      margin: 5px 0;
    }
    .info-content {
      font-size: 14px;
      color: #333;
      margin-bottom: 10px;
    }
    .content-title {
      font-size: 17px;
      font-weight: bold;
      color: #333;
    }
    .content-top {
      width: 100%;
      padding: 15px;
      border-bottom: 1px solid #eee;
      .mid-box {
        width: 100%;
        display: flex;
      }
      .mid-box > div {
        width: 50%;
        padding-right: 5px;
      }
      .info-location {
        width: 100%;
        height: 150px;
        border: 1px solid;
      }
      .imgs-box {
        width: 100%;
        .imgs {
          width: 80px;
          height: 80px;
          margin-right: 5px;
          margin-bottom: 5px;
        }
        .loading-img {
          width: 100%;
          text-align: center;
        }
      }
    }
  }
}
</style>
