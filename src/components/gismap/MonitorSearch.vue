  ...
  <div class="monitor-list">
    ...
    <div class="modal-content" id="modal-content">
      <div class="flex-content">
        ...
        <div class="modal-check states">
          <div :class="checkState === '' ? 'check-btn' : 'nocheck-btn'" @click="changeState('')">全部</div>
          <div v-for="item in stateList" :key="item.code"
            @click="changeState(item.code)"
            :class="checkState === item.code ? 'check-btn' : 'nocheck-btn'">{{item.name}}</div>
        </div>
      </div>
    ...
    <!-- 查询数据 -->
    <div class="data-box">
      <div v-if="list && list.length === 0" class="not-data">
        <div>
      ...
      <van-pull-refresh v-else v-model="refreshing" @refresh="onRefresh">
        <van-list
          ...
          @load="onLoad"
        >
          <div class="data-item" v-for="item in list" :key="item.id" @click="chooseFactory(item)">
...
export default {
  ...
  methods: {
    ...
    getState() {
      this.axios({
        url:"/authenticated/dictionary-types/industryType/roots",
        method:"GET"
      })
      .then(res => {
    ...
    // 选中企业
    chooseFactory(i) {
      sessionStorage.setItem("chooseFactory", JSON.stringify(i));
      this.$router.go(-1);
    }
  },
};
