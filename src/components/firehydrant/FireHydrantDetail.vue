// 消防栓详情
<template>
  <div class="fire-detail">
    <div class="supTop-nav">
      <i class="el-icon-arrow-left" @click="goBack" />
      <div>消防栓详情</div>
      <div @click="editLoLa">编辑</div>
    </div>
    <div class="fire-content" v-if="hasAuth">
      <div class="hydrant-info">
        <div class="info-top">
          <div class="item-area">{{ data.area }}</div>
          <div class="item-state normal" v-if="data.state == 'mfhs001'">
            正常
          </div>
          <div class="item-state danger" v-else-if="data.state == 'mfhs002'">
            损坏
          </div>
          <div class="item-state danger" v-else-if="data.state == 'mfhs003'">
            无水
          </div>
          <div class="item-state danger" v-else-if="data.state == 'mfhs004'">
            锈死
          </div>
          <div class="item-state danger" v-else>顶盖丢失</div>
        </div>
        <div class="hydrant-fa">
          <div class="hydrant-child">
            <div style="margin: 0 0 10px 0">
              <p class="info-title">给水压力</p>
              <p class="info-content">{{ data.pipeNetPressure }}Mpa</p>
            </div>
            <div style="margin: 0 0 10px 0">
              <p class="info-title">规格</p>
              <p class="info-content">
                {{ format.formatObjKey(data.pipeNetShapeDictMap) }}
              </p>
            </div>
          </div>
          <div class="rectify-child">
            <div style="margin: 0 0 10px 0">
              <p class="info-title">管网直径</p>
              <p class="info-content">{{ data.pipeDiameter + "mm" }}</p>
            </div>
            <div style="margin: 0 0 10px 0">
              <p class="info-title">管网类型</p>
              <p class="info-content">
                {{ format.formatObjKey(data.typeDictMap) }}
              </p>
            </div>
          </div>
        </div>
        <div style="margin: 0 0 10px 0">
          <p class="info-title">所在位置</p>
          <p class="info-content">{{ data.location }}</p>
        </div>
        <!-- <el-image class="info-location" :src="imgUrl" /> -->
        <div class="info-location" id="egismap"></div>
      </div>
      <div class="hydetail">
        <div class="detail-top">
          <div class="top-left">
            <span class="info-content">检查明细</span>
            <div @click="dialogVisible = true">
              <span>{{ screenText }}</span>
              <i class="el-icon-caret-bottom"></i>
            </div>
          </div>
          <div class="add-btn" @click="toAddHydrant">新增</div>
        </div>
        <!-- 点击全部-筛选明细 -->
        <el-dialog
          :visible.sync="dialogVisible"
          :before-close="handleClose"
          :show-close="false"
          top="40vh"
          width="80%"
          custom-class="inspect-modal"
        >
          <div class="modal-text" @click="changeScreenText('')">全部</div>
          <div class="modal-text" @click="changeScreenText('mfhcs001')">
            待确认
          </div>
          <div class="modal-text" @click="changeScreenText('mfhcs002')">
            通过
          </div>
          <div
            class="modal-text modal-mid-text"
            @click="changeScreenText('mfhcs003')"
          >
            不通过
          </div>
        </el-dialog>
        <!-- 查询数据 -->
        <div
          class="detail-item"
          @click="toParticular(item)"
          v-for="item in newList"
          :key="item.id"
          :title="item"
        >
          <div class="item-top">
            <div
              class="waitSure"
              v-if="format.formatObjKey(item.checkStateDictMap) === '待确认'"
            >
              待确认
            </div>
            <div
              class="passed"
              v-else-if="format.formatObjKey(item.checkStateDictMap) === '通过'"
            >
              通过
            </div>
            <div class="nopass" v-else>不通过</div>
            <span>{{ item.checker }}</span>
            <span class="item-time">{{ item.checkTime }}</span>
          </div>
          <div class="item-content">{{ item.checkContent }}</div>
        </div>
      </div>
    </div>
    <div v-else class="not-data">
      <div>
        <img src="@/assets/noData.png" alt="" />
        <div class="text">{{ serveText }}</div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "FireHydrantDetail",
  data() {
    return {
      hasAuth: true,
      data: "",
      screenText: "全部", // 筛选
      dialogVisible: false, //
      list: [], // 数据数组
      newList: [],
      longitude: "", // 经度
      latitude: "", // 纬度
      imgUrl: "",
      serveText: ""
    };
  },
  mounted() {
    this.getData();
    this.getDetailById();
  },
  methods: {
    // 返回
    goBack() {
      this.$router.go(-1);
    },
    showMap() {
      let egis = window.egis;
      let that = this;
      var urlWMTS = "http://120.52.31.31:590/service/api/egis/base/v1/wmts"; //wmts服务地址
      var client_id = "4441c1fa1b004b58a4eabdb255d966cc"; //用户id
      var client_secret = "46d3c60424e841d8b0ebb9e0a14cb43e"; //用户密码
      // 创建地图
      var center = [that.longitude, that.latitude];
      var egismap = new egis.carto.Map({
        defaultExtent: {
          center,
          maxZoom: 15,
          minZoom: 15,
          level: 15,
          extent: [-180, -90, 180, 90],
          projection: "EPSG:4490"
        }
      });
      // 初始化地图，传入要初始化的DOM对象的id
      egismap.init({ targetId: "egismap" });
      var resthttp = new egis.core.RestHttp({
        client_id: client_id,
        client_secret: client_secret
      });
      // 创建 天地图矢量 瓦片图层
      var tiandituvec = new egis.carto.TileLayer({
        restHttp: resthttp,
        name: "天地图矢量",
        layers: "vec", //": "图层名称",
        matrix: 21, //": "切图级别小于等于切图级别",
        matrixSet: "c", //": "切图策略",
        matrixPrefix: "", //": "切图策略加冒号：",
        format: "tiles", //": "图层格式",
        projection: "EPSG:4490", //": "投影参考",
        layerType: 1, //": "图层类型",
        tileType: 102, //": "瓦片类型",
        opacity: 1.0, //": "透明度",
        visible: true, //": "是否显示",
        crossOrigin: "anonymous",
        style: "default",
        extent: { minx: -180, miny: -90, maxx: 180, maxy: 90 },
        wrapX: true, //": "是否展示循环图",
        url: urlWMTS
      });

      // 创建 天地图中文标注 瓦片图层
      var tianditucta = new egis.carto.TileLayer({
        restHttp: resthttp,
        name: "天地图中文标注",
        layers: "cva", // 图层名称
        matrixSet: "c", // 切图策略
        format: "tiles", // 图层格式
        projection: "EPSG:4490",
        extent: { minx: -180, miny: -90, maxx: 180, maxy: 90 },
        matrixPrefix: "",
        matrix: 21,
        tileType: 102, // 瓦片类型
        opacity: 1.0,
        visible: true,
        wrapX: true, //": "是否展示循环图",
        url: urlWMTS // 图层服务 url
      });
      //添加天地图矢量图层
      egismap.addLayer(tiandituvec);
      //添加天地图中文标注图层
      egismap.addLayer(tianditucta);
      var elementLayer = new egis.carto.ElementLayer({
        name: "元素图层",
        id: "elementLayer",
        visible: true,
        opacity: 1
      });
      egismap.addLayer(elementLayer);

      var picturemarkersymbol = new egis.sfs.PictureMarkerSymbol({
        source:
          "data:image/png;base64,",
        width: 50,
        height: 60,
        rotation: 0,
        opacity: 1,
        offsetX: 0,
        offsetY: 15,
        scale: 0.5
      });
      //创建图片标记面元素
      var linearElement = new egis.sfs.Element({
        geometry: new egis.sfs.Point({
          x: center[0],
          y: center[1],
          spatialReference: 4490
        }),
        symbol: picturemarkersymbol
      });
      elementLayer.add(linearElement);
      egismap.removeInteractions();
    },
    // 获取数据
    getData() {
      this.axios({
        url: `/municipal/fire/hydrant/${sessionStorage.getItem("hydrantId")}`,
        method: "GET"
      })
        .then(res => {
          // this.hasAuth = true;
          this.data = res.data.data;
          this.longitude = res.data.data.longitude;
          this.latitude = res.data.data.latitude;
          sessionStorage.setItem("hydetail", JSON.stringify(res.data.data));
          this.showMap();
        })
        .catch(err => {
          console.log(err);
          if (err.status === 403) {
            this.hasAuth = false;
            this.serveText = "禁止访问";
          }
        });
    },
    handleClose() {
      this.dialogVisible = false;
    },
    // 修改筛选条件
    changeScreenText(val) {
      if (val === "") {
        this.screenText = "全部";
        this.newList = this.list;
        this.dialogVisible = false;
        return;
      } else if (val === "mfhcs001") {
        this.screenText = "待确认";
      } else if (val === "mfhcs002") {
        this.screenText = "通过";
      } else {
        this.screenText = "不通过";
      }
      this.newList = this.list.filter(item => item.checkState === val);
      this.dialogVisible = false;
    },
    // 新增明细
    toAddHydrant() {
      this.$router.push({
        path: "/addhydrant"
      });
    },
    // 去明细详情
    toParticular(i) {
      sessionStorage.setItem("fhddId", i.id);
      this.$router.push({
        path: "/particularlist"
      });
    },
    // 通过id查询检查明细
    getDetailById() {
      let fireHydrantId = sessionStorage.getItem("hydrantId");
      this.axios({
        url: `/app/municipal/fire/hydrant/check/list/byFireHydrantId/${fireHydrantId}`,
        method: "GET"
      })
        .then(res => {
          this.list = res.data.data;
          this.newList = res.data.data;
        })
        .catch(err => {
          console.log(err);
        });
    },
    editLoLa() {
      this.$router.push({
        path: "/changelola"
      });
    }
  }
};
</script>

<style lang="less">
.fire-detail {
  width: 100vw;
  height: 100vh;
  position: relative;
  display: flex;
  flex-direction: column;
  overflow: hidden;
  background: #eee;
  .el-icon-arrow-left {
    font-size: 20px;
  }
  .supTop-nav {
    background-color: #0199ff;
    height: 50px;
    display: flex;
    align-items: center;
    padding: 0 15px;
    justify-content: space-between;
    color: white;
  }
  .modal-text {
    width: 100%;
    height: 50px;
    line-height: 50px;
    padding-left: 15px;
    border-bottom: 1px solid #d0d0d0;
  }
  .modal-mid-text {
    border-bottom: none;
  }
  .fire-content {
    width: 100%;
    display: flex;
    flex-direction: column;
    flex: 1;
    overflow-x: hidden;
  }
  .hydrant-info {
    width: 100%;
    background: white;
    display: flex;
    padding: 15px;
    flex-direction: column;
    .info-top {
      width: 100%;
      display: flex;
      justify-content: space-between;
      height: 30px;
      margin-bottom: 15px;
      .item-area {
        font-size: 13px;
        width: 75%;
      }
      .item-state {
        height: 20px;
        padding: 0 10px;
        line-height: 20px;
        color: white;
        border-radius: 30px;
        font-size: 12px;
      }
      .danger {
        background: #ff4948;
      }
      .normal {
        background: #1db761;
      }
    }
    .info-location {
      width: 100%;
      height: 150px;
    }
  }
  .hydrant-fa {
    display: flex;
    width: 100%;
    .hydrant-child {
      width: 50%;
      height: 100%;
      display: flex;
      flex-direction: column;
    }
  }
  .info-title {
    color: #9f9f9f;
    font-size: 13px;
    margin-bottom: 5px;
  }
  .info-content {
    font-size: 14px;
    color: #333;
  }
  .hydetail {
    width: 100%;
    display: flex;
    flex-direction: column;
    flex: 1;
    padding: 10px 15px;
    margin-top: 10px;
    background: white;
    .detail-top {
      width: 100%;
      height: 50px;
      display: flex;
      justify-content: space-between;
      align-items: center;
    }
    .top-left {
      display: flex;
      align-items: center;
      height: 100%;
      div {
        margin-left: 10px;
        padding: 0 10px;
        height: 30px;
        border-radius: 15px;
        display: flex;
        justify-content: center;
        align-items: center;
        background: #f5f5f5;
        color: #999;
      }
    }
    .add-btn {
      width: 90px;
      height: 30px;
      line-height: 30px;
      background: #f5f5f5;
      border-radius: 15px;
      text-align: center;
      box-shadow: 0px 2px 3px 1px #d7d7d7;
    }
  }
  .detail-item {
    width: 100%;
    border: 1px solid #ededed;
    border-radius: 10px;
    background: #fbfbfb;
    padding: 10px;
    color: #333;
    margin-top: 10px;
    .item-top {
      width: 100%;
      display: flex;
      margin-bottom: 10px;
      div {
        padding: 0 10px;
        color: white;
        border-radius: 20px;
        height: 25px;
        line-height: 25px;
        font-size: 13px;
        margin-right: 10px;
      }
      .waitSure {
        background: #ff8d1b;
      }
      .passed {
        background: #0199ff;
      }
      .nopass {
        background: rgb(255, 73, 72);
      }
      span {
        display: inline-block;
        height: 100%;
      }
      .item-time {
        margin-left: 10px;
      }
    }
    .item.content {
      width: 100%;
    }
  }
  .modal {
    background: white;
    width: 100%;
    height: 100%;
    border-radius: 5px !important;
  }
  .modal-top {
    width: 100%;
    height: 50px;
    line-height: 50px;
    padding: 0 15px;
    border-bottom: 1px solid #eee;
  }
  .modal-mid {
    width: 100%;
    height: 70vh;
    display: flex;
    flex-direction: column;
    div {
      width: 100%;
      display: flex;
      align-items: center;
      margin-bottom: 10px;
    }
    .mid-title {
      font-weight: bold;
    }
    .search-input {
      border: none;
      width: 85%;
      height: 100%;
      background: #f2f2f2;
      border-radius: 5px;
      padding-left: 10px;
      height: 45px;
    }
  }
  .modal-foot {
    width: 100%;
    height: 50px;
    display: flex;
    border-top: 1px solid #eee;
    color: rgb(68, 153, 246);
    div {
      width: 50%;
      height: 100%;
      line-height: 50px;
      text-align: center;
    }
    .cancel {
      border-left: 1px solid #eee;
    }
  }
}
</style>
